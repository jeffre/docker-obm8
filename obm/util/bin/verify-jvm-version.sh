...
print_usage()
{
    DEBUG echo "Usage: \"$FILENAME_CURRENT_SCRIPT\" \"JVM Home Dir Path\" \"Minimum Supported JVM Version\" \"Maximum Supported JVM Version\""
    DEBUG echo "Verify the current JVM version if it meets the minimum (and maximum) supported version."
}
...
is_jvm_versions_format_valid()
{
    ...
    then
        ...
        if [ ! -z "$MAX_SUPPORTED_JVM_VERSION" ]
        then
            echo "$MAX_SUPPORTED_JVM_VERSION" | egrep "[0-9]+\.[0-9]+.*" 1>"/dev/null"
            if [ $? -eq 0 ]
            then
                return 0
            else
                 DEBUG echo "Provided Maximum supported JVM version \"$MAX_SUPPORTED_JVM_VERSION\" is NOT valid!"
                 DEBUG echo "Please given a valid maximum supported JVM version!"
                 return 2
            fi
        fi
    else
...
parse_min_supported_jvm_version_tokens()
{
...
parse_max_supported_jvm_version_tokens()
{
    MAX_SUPPORTED_JVM_VERSION_MAJ_VER_NUM=`echo "$MAX_SUPPORTED_JVM_VERSION" | cut -f1 -d"."`
    MAX_SUPPORTED_JVM_VERSION_MINOR_VER_NUM=`echo "$MAX_SUPPORTED_JVM_VERSION" | cut -f2 -d"."`
    MAX_SUPPORTED_JVM_VERSION_MINOR2_VER_NUM=`echo "$MAX_SUPPORTED_JVM_VERSION" | cut -f3 -d"." | cut -f1 -d"-" | cut -f1 -d"_"`
    [ "$MAX_SUPPORTED_JVM_VERSION_MINOR2_VER_NUM" = "" ] && MAX_SUPPORTED_JVM_VERSION_MINOR2_VER_NUM=0
    if [ `echo "$MAX_SUPPORTED_JVM_VERSION" | grep "_" | grep -cv "grep _"` -ge 1 ]
    then
        MAX_SUPPORTED_JVM_VERSION_UPDATE_VER_NUM=`echo "$MAX_SUPPORTED_JVM_VERSION" | cut -f2 -d"_" | cut -f1 -d"-"`
    else
        MAX_SUPPORTED_JVM_VERSION_UPDATE_VER_NUM=0
    fi

    MAX_SUPPORTED_JVM_VERSION_SPACE_SEPEARATED="${MAX_SUPPORTED_JVM_VERSION_MAJ_VER_NUM} ${MAX_SUPPORTED_JVM_VERSION_MINOR_VER_NUM} ${MAX_SUPPORTED_JVM_VERSION_MINOR2_VER_NUM} ${MAX_SUPPORTED_JVM_VERSION_UPDATE_VER_NUM}"
}
...
parse_jvm_version_tokens()
{
    parse_current_jvm_version_tokens
    parse_min_supported_jvm_version_tokens
    # 2015-11-18 (sunny.pan) Added Maximum supported version for range checking
    if [ ! -z "$MAX_SUPPORTED_JVM_VERSION" ]
    then
        parse_max_supported_jvm_version_tokens
    fi
...
is_jvm_version_supported()
{
    ...
    do
        ...
        if [ $TOKEN_CURRENT_JVM_VERSION -ge $TOKEN_MIN_SUPPORTED_JVM_VERSION ]
        then
            FLAG_IS_JVM_VERSION_SUPPORTED=0
            # 2015-11-18 (sunny.pan) Added Maximum supported version for range checking
            if [ ! -z "$MAX_SUPPORTED_JVM_VERSION_SPACE_SEPEARATED" ]
            then
                TOKEN_MAX_SUPPORTED_JVM_VERSION=`echo "$MAX_SUPPORTED_JVM_VERSION_SPACE_SEPEARATED" | cut -f$COUNT_TOKEN_JVM_VERSION_WORKING -d" "`
                if [ $TOKEN_CURRENT_JVM_VERSION -gt $TOKEN_MAX_SUPPORTED_JVM_VERSION ]
                then
                    FLAG_IS_JVM_VERSION_SUPPORTED=2
                    break
                fi
            fi
            break
...
if [ ! -z "$3" ]
then
    MAX_SUPPORTED_JVM_VERSION=$3
fi
echo "MAX_SUPPORTED_JVM_VERSION = $MAX_SUPPORTED_JVM_VERSION"
